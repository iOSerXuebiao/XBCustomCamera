...
+ (NSString *)pathForApplicationSupportDirectoryWithPath:(NSString *)path {
    return [[XBFileManager pathForApplicationSupportDirectory] stringByAppendingPathComponent:path];
}
...
+ (NSString *)pathForCachesDirectoryWithPath:(NSString *)path {
    return [[XBFileManager pathForCachesDirectory] stringByAppendingPathComponent:path];
}
...
+ (NSString *)pathForDocumentsDirectoryWithPath:(NSString *)path {
    return [[XBFileManager pathForDocumentsDirectory] stringByAppendingPathComponent:path];
}
...
+ (NSString *)pathForMainBundleDirectoryWithPath:(NSString *)path {
    return [[XBFileManager pathForMainBundleDirectory] stringByAppendingPathComponent:path];
}
...
+ (NSString *)pathForTemporaryDirectoryWithPath:(NSString *)path {
    return [[XBFileManager pathForTemporaryDirectory] stringByAppendingPathComponent:path];
}
...
+ (void)archiveFileAtPath:(NSString *)path content:(id<NSCoding>)content {
    return [self archiveFileAtPath:path content:content error:nil];
}

+ (void)archiveFileAtPath:(NSString *)path content:(id<NSCoding>)content error:(NSError **)error {
    if (content == nil) {
        //        [NSException raise:@"Invalid content" format:@"content can't be nil."];
        return;
    }
...
+ (id<NSCoding>)unarchiveFileAtPath:(NSString *)path {
    NSString *absolutePath = [self absolutePath:path];
    return [NSKeyedUnarchiver unarchiveObjectWithFile:absolutePath];
}
...
//remove file
+ (void)removeFileCache:(NSString *)path exceptFileNames:(NSArray *)whiteFileNames {
    NSArray *files = [[NSFileManager defaultManager] subpathsAtPath:path];
    NSString *tmpfilePath = @"";
    for (NSString *filePath in files) {
        tmpfilePath  = [NSString stringWithString:filePath];
        for (NSString *whiteFileTmp in whiteFileNames) {
            //如果不需要删除的
            if ([filePath hasPrefix:whiteFileTmp]) {
                tmpfilePath = @"";
            }
        }
        if (tmpfilePath.length > 0) {
        NSError *error;
        NSString *subPath = [path stringByAppendingPathComponent:filePath];
...
//remove file
+ (void)removeFileCache:(NSString *)path exceptFileNames:(NSArray *)whiteFileNames error:(NSError *)error {
    NSArray *files = [[NSFileManager defaultManager] subpathsAtPath:path];
    NSString *tmpfilePath = @"";
    for (NSString *filePath in files) {
        tmpfilePath  = [NSString stringWithString:filePath];
